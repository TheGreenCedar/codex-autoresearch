...
node plugins/codex-autoresearch/scripts/autoresearch.mjs serve --cwd plugins/codex-autoresearch
```
...
node <plugin-root>/scripts/autoresearch.mjs serve --cwd <current-project>
...
`serve` is intentionally long-running. Keep it open in a terminal or tool session, then run `next` and `log` commands from another terminal or through MCP/live actions.

After any start or resume path, start the live dashboard first and directly provide its `http://127.0.0.1:<port>/` URL before continuing with experiments or status narration. Treat `autoresearch-dashboard.html` as a fallback snapshot for offline review only; do not hand it to the operator when a live URL can be served.
...
- `export`: use only when a read-only offline snapshot is explicitly needed. Run `node <plugin-root>/scripts/autoresearch.mjs export --cwd <current-project>` and report that it is not the live operator dashboard.
...
- `serve`: run `node <plugin-root>/scripts/autoresearch.mjs serve --cwd <current-project>` to start the local live dashboard and safe action endpoints. Report the returned URL as the normal dashboard link.
...
- Any other text: use the `autoresearch-create` skill to start or resume the loop using the text as the goal/context, then start or reuse the live dashboard and directly provide the live URL.
...
Dashboard modes must stay explicit. The served `http://127.0.0.1:<port>/` dashboard is the normal operator surface and the link to provide by default. The exported `autoresearch-dashboard.html` file is a static, read-only fallback snapshot with no inert live controls and no copyable command panel. Dashboard live actions are local-only and limited to bounded actions: doctor, setup-plan, recipes, gap-candidates preview, finalize-preview, export, and confirmed log decisions with a specific description. Mutating review branch creation remains outside the dashboard surface.
