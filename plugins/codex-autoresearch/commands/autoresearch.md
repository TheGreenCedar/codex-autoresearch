...
Dashboard modes must stay explicit. The served `http://127.0.0.1:<port>/` dashboard is the normal operator surface and the link to provide by default. The exported `autoresearch-dashboard.html` file is a static, read-only fallback snapshot with copyable commands and no inert live controls. Dashboard live actions are local-only and limited to bounded commands: doctor, setup-plan, recipes, gap-candidates preview, finalize-preview, export, and confirmed log decisions with a specific description. Mutating review branch creation remains outside the dashboard surface.
