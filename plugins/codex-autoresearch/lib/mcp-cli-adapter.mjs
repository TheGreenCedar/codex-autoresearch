...
function cliArgsForTool(name, args) {
  ...
  if (name === "guided_setup") return compactArgs(["guide", cwdFlag(args), option("--recipe", args.recipe_id ?? args.recipeId ?? args.recipe), option("--catalog", args.catalog), option("--name", args.name), option("--metric-name", args.metric_name ?? args.metricName), option("--benchmark-command", args.benchmark_command ?? args.benchmarkCommand)]);
