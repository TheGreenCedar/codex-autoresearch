...
function cliArgsForTool(name, args) {
  if (name === "setup_plan") return compactArgs(["setup-plan", cwdFlag(args), option("--recipe", args.recipe_id ?? args.recipeId ?? args.recipe), option("--catalog", args.catalog), option("--name", args.name), option("--metric-name", args.metric_name ?? args.metricName), option("--benchmark-command", args.benchmark_command ?? args.benchmarkCommand), option("--checks-command", args.checks_command ?? args.checksCommand), listOption("--commit-paths", args.commit_paths ?? args.commitPaths), option("--max-iterations", args.max_iterations ?? args.maxIterations)]);
  if (name === "guided_setup") return compactArgs(["guide", cwdFlag(args), option("--recipe", args.recipe_id ?? args.recipeId ?? args.recipe), option("--catalog", args.catalog), option("--name", args.name), option("--metric-name", args.metric_name ?? args.metricName), option("--benchmark-command", args.benchmark_command ?? args.benchmarkCommand), option("--checks-command", args.checks_command ?? args.checksCommand), listOption("--commit-paths", args.commit_paths ?? args.commitPaths), option("--max-iterations", args.max_iterations ?? args.maxIterations)]);
  ...
  if (name === "doctor_session") return compactArgs(["doctor", cwdFlag(args), option("--command", args.command), flag("--check-benchmark", args.check_benchmark ?? args.checkBenchmark), flag("--check-installed", args.check_installed ?? args.checkInstalled), option("--timeout-seconds", args.timeout_seconds ?? args.timeoutSeconds)]);
  if (name === "clear_session") return compactArgs(["clear", cwdFlag(args), flag("--dry-run", args.dry_run ?? args.dryRun), flag("--yes", args.confirm ?? args.yes)]);
