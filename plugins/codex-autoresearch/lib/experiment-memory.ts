  ...
  const aggregateLanes = [
    {
      ...
      nextActionHint:
        kept.at(-1)?.asi?.next_action_hint ||
        kept.at(-1)?.description ||
    ...
    },
    {
      ...
      reason:
        rejected.at(-1)?.asi?.rollback_reason ||
        rejected.at(-1)?.description ||
        "No rejected lane yet.",
      nextActionHint:
        rejected.at(-1)?.asi?.next_action_hint ||
        rejected.at(-1)?.asi?.rollback_reason ||
        rejected.at(-1)?.description ||
  ...
  return [
    {
      ...
      reason: plateau.detected
        ...
        : "Reserve one lane for a materially different family each batch.",
      nextActionHint:
    ...
    },
    {
      ...
      reason: exhaustedFamily
        ? `${exhaustedFamily.label} looks exhausted.`
        : "Small tweaks are useful after the portfolio has enough novelty.",
    ...
    },
    {
      ...
      reason: recentFailures
        ? `${recentFailures} recent failed or discarded run(s) need a different blocker hypothesis.`
        : "Use this lane when failures share a cause.",
      nextActionHint:
    ...
    ...aggregateLanes,
    {
      ...
      status: "ready",
      reason:
        "Always reserve one slot for a non-local solution that could change the search space.",
