...
const DASHBOARD_ACTIONS = new Map([
  ...
  ["log-keep", { className: "log-decision", allowedBodyKeys: ["confirm", "lastRunFingerprint", "description", "asi"] }],
  ["log-discard", { className: "log-decision", allowedBodyKeys: ["confirm", "lastRunFingerprint", "description", "asi"] }],
  ["log-crash", { className: "log-decision", allowedBodyKeys: ["confirm", "lastRunFingerprint", "description", "asi"] }],
  ["log-checks-failed", { className: "log-decision", allowedBodyKeys: ["confirm", "lastRunFingerprint", "description", "asi"] }],
]);
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "GET" && url.pathname === "/") {
        send(res, 200, "text/html; charset=utf-8", await dashboardHtml({ actionNonce, actionNonceHeader: "X-Autoresearch-Action-Nonce" }));
        return;
      }
      ...
      if (req.method === "POST" && url.pathname.startsWith("/actions/")) {
        ...
        try {
          ...
          sendJson(res, actionErrorEnvelope(action, error.message || String(error), error.code || "dashboard_action_failed"), error.statusCode || 500);
        }
        return;
      }
      sendJson(res, { ok: false, error: "Not found" }, 404);
    } catch (error) {
      sendJson(res, actionErrorEnvelope("dashboard", error.message || String(error), error.code || "dashboard_action_failed"), error.statusCode || 500);
    }
  });
...
async function logActionArgs(action, workDir, body) {
  ...
  if (!description || /^Describe the /.test(description)) {
    throw new DashboardActionError("Log actions require a specific description.", 400, "log_description_required");
  }
...
function validateActionRequest(req, { actionNonce }) {
  ...
  if (nonce !== actionNonce) {
    return { ok: false, status: 403, code: "action_nonce_invalid", error: "Live action token is missing or invalid." };
  }
  const host = String(req.headers.host || "");
  if (!/^(127\.0\.0\.1|localhost)(:\d+)?$/i.test(host)) {
    return { ok: false, status: 403, code: "host_forbidden", error: "Live actions are only available on the loopback dashboard host." };
  }
  const origin = String(req.headers.origin || "");
  if (origin && !sameLoopbackOrigin(origin, host)) {
    return { ok: false, status: 403, code: "origin_forbidden", error: "Cross-origin dashboard actions are not allowed." };
  }
  const referer = String(req.headers.referer || "");
  if (!origin && referer) {
    try {
      if (!sameLoopbackOrigin(new URL(referer).origin, host)) {
        return { ok: false, status: 403, code: "referer_forbidden", error: "Cross-origin dashboard actions are not allowed." };
      }
    } catch {
      return { ok: false, status: 403, code: "referer_invalid", error: "Invalid dashboard action referer." };
    }
  }
  if (String(req.headers["sec-fetch-site"] || "").toLowerCase() === "cross-site") {
    return { ok: false, status: 403, code: "fetch_site_forbidden", error: "Cross-site dashboard actions are not allowed." };
  }
  const contentType = String(req.headers["content-type"] || "").toLowerCase();
  if (!contentType.startsWith("application/json")) {
    return { ok: false, status: 415, code: "unsupported_media_type", error: "Dashboard actions require application/json." };
  }
  const contentLength = Number(req.headers["content-length"]);
  if (Number.isFinite(contentLength) && contentLength > ACTION_BODY_MAX_BYTES) {
    return { ok: false, status: 413, code: "body_too_large", error: `Dashboard action bodies are limited to ${ACTION_BODY_MAX_BYTES} bytes.` };
  }
  return { ok: true };
}
...
function validateActionBody(action, policy, body) {
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    throw new DashboardActionError("Dashboard action body must be a JSON object.", 400, "body_not_object");
  }
  const allowed = new Set(policy.allowedBodyKeys || []);
  for (const key of Object.keys(body)) {
    if (!allowed.has(key)) {
      throw new DashboardActionError(`Dashboard action body field is not allowed: ${key}.`, 400, "body_field_forbidden");
    }
  }
  if (action === "gap-candidates") {
    for (const key of ["apply", "model_command", "modelCommand", "allow_unsafe_command", "allowUnsafeCommand", "command", "checks_command"]) {
      if (Object.hasOwn(body, key)) {
        throw new DashboardActionError(`Dashboard gap preview cannot accept ${key}.`, 400, "body_field_forbidden");
      }
    }
  }
}
...
async function assertLastRunFingerprint(workDir, submitted) {
  if (!submitted) {
    throw new DashboardActionError("Log actions require the current last-run fingerprint.", 400, "last_run_fingerprint_required");
  }
  const current = await currentLastRunFingerprint(workDir);
  if (!current) {
    throw new DashboardActionError("No last-run packet is available to log.", 409, "last_run_missing");
  }
  if (submitted !== current) {
    throw new DashboardActionError("Last-run packet changed. Refresh the dashboard before logging.", 409, "last_run_stale");
  }
}

async function currentLastRunFingerprint(workDir) {
  const candidates = [];
  const gitPath = await runProcess("git", ["rev-parse", "--git-path", "autoresearch/last-run.json"], {
    cwd: workDir,
    timeoutSeconds: 5,
    maxOutputBytes: 4096,
  }).catch(() => null);
