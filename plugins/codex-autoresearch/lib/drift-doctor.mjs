...
export async function inspectVersionSurfaces({ pluginRoot }) {
  ...
  const warnings = unique.length > 1
    ? [`Local version surfaces disagree: ${Object.entries(surfaces).map(([key, value]) => `${key}=${value || "missing"}`).join(", ")}.`]
    : [];
  return {
...
export async function inspectInstalledRouting({ pluginName = "codex-autoresearch", timeoutMs = 5000 } = {}) {
  ...
  const pathMatch = output.match(/[A-Z]:\\[^\r\n"]*codex-autoresearch[^\r\n"]*/i)
    || output.match(/\/[^\r\n"]*codex-autoresearch[^\r\n"]*/i);
  const versionMatch = output.match(/codex-autoresearch[\\/](\d+\.\d+\.\d+)/i)
...
export async function buildDriftReport({ pluginRoot, includeInstalled = false } = {}) {
  ...
  if (includeInstalled) {
    ...
    if (!installed.available) {
      ...
      report.warnings.push(`Installed codex-autoresearch appears to be ${installed.version}, while local source is ${local.version}.`);
    }
  }
