...
export function createCliToolCaller({
  ...
  const waitForServePayload = async (child, stdoutFn, stderrFn) => {
    ...
    while (Date.now() - started < 5000) {
      ...
      if (child.exitCode != null) {
        throw new Error(
          `autoresearch live dashboard exited (${child.exitCode})\n${stderrFn() || stdoutFn()}`,
        );
      }
  ...
  const startLiveDashboard = async (args) => {
    ...
    const payload = await waitForServePayload(
      child,
      () => stdout,
      () => stderr,
    );
  ...
  return async function callCliTool(name, args) {
    ...
    if (result.code !== 0) {
      throw new Error(
        `autoresearch CLI failed (${result.code})\n${result.stderr || result.stdout}`,
      );
    }
    try {
...
function cliArgsForTool(name, args) {
  ...
  if (name === "setup_research_session")
    return compactArgs([
      "research-setup",
      cwdFlag(args),
  ...
  if (name === "serve_dashboard")
    return compactArgs(["serve", cwdFlag(args), option("--port", args.port)]);
