...
async function writeFile$1(file, contents) {
...
test("finalizer writes an ignored review summary and preserves verification", async () => {
	...
	await writeFile$1(path.join(repo, "src", "value.txt"), "base\n");
	await writeFile$1(path.join(repo, "src", "other.txt"), "base other\n");
	...
	await git([
		"switch",
		"-c",
		"codex/autoresearch-test"
	], repo);
	await writeFile$1(path.join(repo, "src", "space path.txt"), "kept\n");
	await writeFile$1(path.join(repo, "scripts", "autoresearch.ts"), "console.log('legitimate source change');\n");
	await writeFile$1(path.join(repo, "autoresearch-dashboard.html"), "<html>ignored export</html>\n");
	await writeFile$1(path.join(repo, "autoresearch.md"), "# session\n");
	await writeFile$1(path.join(repo, "autoresearch.research", "study", "quality-gaps.md"), "- [ ] session scratchpad\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
	...
	const summaryPath = result.stdout.split(/\r?\n/).find((line) => line.startsWith("Review summary: ")).slice(16).trim();
	...
	assert.match(summary, /Final file set: .*scripts\/autoresearch\.ts.*src\/space path\.txt|Final file set: .*src\/space path\.txt.*scripts\/autoresearch\.ts/);
	...
	const branchFiles = (await git([
		...
		"--name-only",
		"--format=",
		"autoresearch-review/ux-test/01-value-change"
	], repo)).stdout;
...
test("finalizer rejects crafted plan paths before filesystem deletion", async () => {
	...
	await writeFile$1(path.join(repo, "src", "value.txt"), "kept\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
...
test("finalizer plan keeps only kept commits and flags excluded history", async () => {
	...
	await writeFile$1(path.join(repo, "src", "base.txt"), "base\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
	...
	await writeFile$1(path.join(repo, "src", "kept.txt"), "kept\n");
	await writeFile$1(path.join(repo, "autoresearch-dashboard.html"), "<html>ignored export</html>\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
	...
	await writeFile$1(path.join(repo, "src", "discarded.txt"), "discarded\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
		"discard value change"
	], repo);
	const discardHash = (await git(["rev-parse", "HEAD"], repo)).stdout.trim();
	await writeFile$1(path.join(repo, "src", "crash.txt"), "crash\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
	...
	await writeFile$1(path.join(repo, "src", "unlogged.txt"), "unlogged\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
		"unlogged value change"
	], repo);
	const unloggedHash = (await git(["rev-parse", "HEAD"], repo)).stdout.trim();
	await writeFile$1(path.join(repo, "autoresearch.jsonl"), [
		JSON.stringify({
			type: "config",
			name: "speed loop",
			metricName: "seconds",
...
test("finalizer plan recommends collapsing overlap and can collapse on request", async () => {
	...
	await git([
		"switch",
		"-c",
		"codex/autoresearch-overlap"
	], repo);
	await writeFile$1(path.join(repo, "src", "value.txt"), "first\n");
	await writeFile$1(path.join(repo, "src", "other.txt"), "first other\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
	...
	await writeFile$1(path.join(repo, "src", "value.txt"), "second\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
	...
	const summary = await fsp.readFile(summaryLine.slice(16).trim(), "utf8");
	assert.match(summary, /Status: verified/);
	const branchFiles = (await git([
		"show",
		"--name-only",
...
test("collapsed finalizer fails closed when excluded commits touch planned kept files", async () => {
	...
	await writeFile$1(path.join(repo, "src", "a.txt"), "base a\n");
	await writeFile$1(path.join(repo, "src", "x.txt"), "base x\n");
	await writeFile$1(path.join(repo, "src", "c.txt"), "base c\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
	...
	await git([
		"switch",
		"-c",
		"codex/autoresearch-collapse-conflict"
	], repo);
	await writeFile$1(path.join(repo, "src", "a.txt"), "kept a\n");
	await writeFile$1(path.join(repo, "src", "x.txt"), "first x\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
	...
	const firstHash = (await git(["rev-parse", "HEAD"], repo)).stdout.trim();
	await writeFile$1(path.join(repo, "src", "a.txt"), "discarded a\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
	...
	await writeFile$1(path.join(repo, "src", "x.txt"), "second x\n");
	await writeFile$1(path.join(repo, "src", "c.txt"), "kept c\n");
	await git(["add", "-A"], repo);
	await git([
		"commit",
		"-m",
	...
	const reviewBranches = (await git([
		"branch",
		"--list",
		"autoresearch-review/*"
	], repo)).stdout.trim();
...
test("finalizer surfaces corrupt autoresearch.jsonl with an actionable error", async () => {
	...
	await writeFile$1(path.join(repo, "autoresearch.jsonl"), [JSON.stringify({
		type: "config",
		name: "speed loop",
		metricName: "seconds",
		bestDirection: "lower"
