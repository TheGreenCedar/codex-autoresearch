...
//#region tests/autoresearch-cli.test.ts
...
const processResult = (code, stdout, stderr) => ({
	code,
	stdout,
	stderr
});
...
async function renderExportedDashboard(html) {
	...
	while (!dom.window.__AUTORESEARCH_DASHBOARD_READY__) {
		if (Date.now() - started > 2e3) throw new Error("Dashboard React app did not finish rendering.");
...
test("run reports missing primary metric as a failed experiment", async () => {
	await withTempDir("missing-metric", async (dir) => {
		...
		const result = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "console.log('no metric here')"`
		]);
...
test("run returns explicit keep/discard decision options instead of a fake status", async () => {
	await withTempDir("decision-hint", async (dir) => {
		...
		const result = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "console.log('METRIC seconds=1.25')"`
		]);
		assert.equal(result.code, 0, result.stderr);
		const payload = JSON.parse(result.stdout);
...
test("state supports negative metrics when lower is better", async () => {
	await withTempDir("negative-metric", async (dir) => {
		...
		assert.equal(exportResult.code, 0, exportResult.stderr);
		const dom = await renderExportedDashboard(await readFile(path.join(dir, "autoresearch-dashboard.html"), "utf8"));
...
test("state reports corrupt JSONL with the ledger path", async () => {
	await withTempDir("state-corrupt-jsonl", async (dir) => {
		await writeFile(path.join(dir, "autoresearch.jsonl"), [JSON.stringify({
			type: "config",
			name: "corrupt state",
			metricName: "seconds"
		}), "{ not valid json"].join("\n") + "\n", "utf8");
...
test("discarded metrics do not become best or suppress on-improvement checks", async () => {
	await withTempDir("discarded-best", async (dir) => {
		...
		const result = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "console.log('METRIC seconds=7')"`,
			"--checks-policy",
			"on-improvement"
		]);
		assert.equal(result.code, 0, result.stderr);
...
test("next writes a reusable last-run packet and log can consume it", async () => {
	await withTempDir("last-run", async (dir) => {
		...
		const next = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "console.log('METRIC seconds=3'); console.log('METRIC cache_hits=8')"`,
			"--checks-policy",
			"manual"
		]);
...
test("successful last-run packets require explicit status and suggest discard for regressions", async () => {
	await withTempDir("last-run-suggest-discard", async (dir) => {
		...
		const next = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "console.log('METRIC seconds=4')"`,
			"--checks-policy",
			"manual"
		]);
		assert.equal(next.code, 0, next.stderr);
...
test("log from last packet rejects keep after failed checks", async () => {
	await withTempDir("last-run-check-failure", async (dir) => {
		...
		const next = await runCli([
			...
			`${quoteForShell(process.execPath)} -e "console.log('METRIC seconds=3')"`,
			"--checks-command",
			`${quoteForShell(process.execPath)} -e "process.exit(1)"`
		]);
		assert.equal(next.code, 0, next.stderr);
		const packet = JSON.parse(next.stdout);
...
test("legacy failed sentinel metrics do not suppress next-run baseline guidance", async () => {
	await withTempDir("legacy-sentinel-baseline", async (dir) => {
		...
		const next = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "console.log('METRIC seconds=5')"`,
			"--checks-policy",
			"manual"
		]);
		assert.equal(next.code, 0, next.stderr);
...
test("export treats missing keep commits as finalization backlog instead of trust warnings", async () => {
	await withTempDir("missing-keep-commit-preview", async (dir) => {
		...
		const previewPacket = exportPayload.viewModel.finalizationChecklist.find((item) => item.label === "Preview packet");
...
test("invalid iteration limits and negative extensions fail loudly", async () => {
	await withTempDir("invalid-iteration-limits", async (dir) => {
		...
		const fractionalExtend = await runCli([
			"config",
			"--cwd",
			dir,
			"--extend",
...
test("mcp tools expose guidance and output contracts", async () => {
	const [{ mcpToolSchemasWithContracts, toolSchemas }, { validateToolContracts }, { cliCommandForTool, toolMutates, validateToolRegistry }] = await Promise.all([
		import("../lib/mcp-interface.mjs"),
		import("../lib/tool-contracts.mjs"),
		import("../lib/tool-registry.mjs")
	]);
...
test("mcp server dispatches tool calls through the CLI wrapper", async () => {
	...
	send({
		...
		params: {
			name: "setup_plan",
			arguments: { working_dir: pluginRoot }
		}
	});
...
test("mcp server dispatches guided setup through the CLI wrapper", async () => {
	await withTempDir("mcp-guided-setup", async (dir) => {
		...
		assert.equal(response.result?.isError, void 0);
		const payload = JSON.parse(response.result.content[0].text);
		assert.equal(payload.ok, true);
...
test("mcp server rejects unknown arguments and gated command materialization", async () => {
	await withTempDir("mcp-contract-rejections", async (dir) => {
		const unknown = await callMcpTool("setup_plan", {
			working_dir: dir,
			typo_argument: true
		});
		...
		assert.equal(readOnlyPlan.result?.isError, void 0, readOnlyPlan.result?.content?.[0]?.text);
		...
		assert.equal(readOnlyGuide.result?.isError, void 0, readOnlyGuide.result?.content?.[0]?.text);
		...
		assert.equal(allowedCatalog.result?.isError, void 0, allowedCatalog.result?.content?.[0]?.text);
...
test("mcp quality-gap tools infer active research slug deterministically", async () => {
	await withTempDir("mcp-quality-gap-slug", async (dir) => {
		...
		assert.equal(measured.result?.isError, void 0, measured.result?.content?.[0]?.text);
		...
		assert.equal(candidates.result?.isError, void 0, candidates.result?.content?.[0]?.text);
		...
		assert.equal(explicit.result?.isError, void 0, explicit.result?.content?.[0]?.text);
...
test("mcp CLI adapter forwards schema-supported options that need CLI flags", async () => {
	...
	await withTempDir("mcp-cli-adapter", async (dir) => {
		...
		assert.deepEqual(guided.args.slice(0, 3), [
			"guide",
			"--cwd",
			dir
		]);
		...
		assert.deepEqual(setupPlan.args.slice(0, 3), [
			"setup-plan",
			"--cwd",
			dir
		]);
		...
		assert.deepEqual(clear.args.slice(0, 3), [
			"clear",
			"--cwd",
			dir
		]);
		...
		const exported = await callTool("export_dashboard", {
			working_dir: dir,
			full: true
		});
		...
		const appliedInvocation = buildCliInvocationForTool("gap_candidates", {
			working_dir: dir,
			apply: true
		}, {
			cliScript: fakeCli,
...
test("large benchmark output is capped and marked truncated", async () => {
	await withTempDir("large-output", async (dir) => {
		...
		const result = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "console.log('x'.repeat(30000)); console.log('METRIC seconds=1')"`
		]);
		assert.equal(result.code, 0, result.stderr);
		const payload = JSON.parse(result.stdout);
		assert.equal(payload.outputTruncated, true);
		assert.ok(payload.tailOutput.length < 9e3);
...
test("large no-newline benchmark tails do not hide early metrics", async () => {
	await withTempDir("large-no-newline-output", async (dir) => {
		...
		const result = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "process.stdout.write('METRIC seconds=2\\n'); process.stdout.write('x'.repeat(300000))"`
		]);
		assert.equal(result.code, 0, result.stderr);
		const payload = JSON.parse(result.stdout);
		assert.equal(payload.outputTruncated, true);
...
test("large metric streams retain bounded metrics and primary evidence", async () => {
	await withTempDir("large-metric-stream", async (dir) => {
		...
		const result = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "for (let i = 0; i < 20000; i++) console.log('METRIC m' + i + '=' + i); console.log('METRIC seconds=1')"`
		]);
		assert.equal(result.code, 0, result.stderr);
		const payload = JSON.parse(result.stdout);
...
test("large metric streams keep a primary metric outside retained output tails", async () => {
	await withTempDir("large-metric-primary-middle", async (dir) => {
		...
		const result = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} ${quoteForShell(emitter)}`
		]);
		assert.equal(result.code, 0, result.stderr);
		const payload = JSON.parse(result.stdout);
		assert.equal(payload.ok, true);
...
test("next command runs preflight and benchmark as one decision packet", async () => {
	await withTempDir("next-command", async (dir) => {
		...
		const result = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "console.log('METRIC seconds=2')"`
		]);
		assert.equal(result.code, 0, result.stderr);
		const payload = JSON.parse(result.stdout);
		assert.equal(payload.ok, true);
...
test("doctor summarizes readiness and detects missing benchmark metrics", async () => {
	await withTempDir("doctor", async (dir) => {
		...
		const result = await runCli([
			...
			dir,
			"--command",
			`${quoteForShell(process.execPath)} -e "console.log('no metric')"`,
			"--check-benchmark"
		]);
		assert.equal(result.code, 0, result.stderr);
		const payload = JSON.parse(result.stdout);
...
test("drift report warns when installed Codex MCP runtime lags source", async () => {
	const { buildDriftReport } = await import("../lib/drift-doctor.mjs");
...
test("runShell configures a POSIX process group for timeout cleanup", async () => {
	const [shim, runner] = await Promise.all([readFile(cli, "utf8"), readFile(path.join(pluginRoot, "lib", "runner.ts"), "utf8")]);
