#!/usr/bin/env node
import { mcpToolSchemas, requireUnsafeCommandGate, validateToolArguments } from "../lib/mcp-tool-schemas.mjs";
...
//#region scripts/autoresearch-mcp.ts
...
async function handleMcpMessage(message) {
	...
	if (message.method === "tools/call") {
		try {
			...
			const normalizedArgs = validateToolArguments(name, message.params?.arguments || {});
			requireUnsafeCommandGate(name, normalizedArgs, boolOption);
			const payload = mcpSuccessEnvelope(name, await callCliTool(name, normalizedArgs));
			sendMcp({
				jsonrpc: "2.0",
				id: message.id,
				result: {
