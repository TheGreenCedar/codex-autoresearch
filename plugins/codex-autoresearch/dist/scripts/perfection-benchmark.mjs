...
//#region scripts/perfection-benchmark.ts
...
async function readDashboardSurface() {
	...
	return (await Promise.all(files.map(readOptionalText))).join("\n");
}
...
async function markdownFilesUnder(dir) {
	...
	try {
		return (await fsp.readdir(absolute, { withFileTypes: true })).filter((entry) => entry.isFile() && entry.name.endsWith(".md")).map((entry) => path.join(dir, entry.name));
	} catch {
		return [];
	}
}
...
const checks = [
	...
	},
	{
		...
		run: async () => {
			const server = (await readJson(".mcp.json")).mcpServers?.["codex-autoresearch"];
			if (!server) return fail("codex-autoresearch MCP server is missing");
			const args = Array.isArray(server.args) ? server.args.join(" ") : "";
			const note = String(server.note || "");
			if (server.cwd === "." && args.includes("./scripts/autoresearch-mcp.mjs") && Number(server.startup_timeout_sec) >= 30 && note.includes("next_experiment") && note.includes("setup_research_session") && note.includes("measure_quality_gap")) return pass();
	...
	},
	{
		...
		run: async () => {
			...
			if (files.length === 1 && files[0] === "skills/codex-autoresearch/SKILL.md" && commandMarkdown.length === 0 && skill.includes("name: codex-autoresearch") && skill.includes("only Codex-facing skill") && agents.includes("display_name: \"Codex Autoresearch\"")) return pass();
	...
	},
	{
		...
		run: async () => {
			...
			]) && !root.includes("static report screenshot") && !root.includes("Static read-only export") ? pass() : fail(pluginReadmeExists || demoReadmeExists ? "A non-root README still exists." : "Root README is missing the public product workflow, live dashboard screenshot, demo, or docs links.");
		}
	},
	{
	...
	},
	{
		...
		description: "Detailed guidance lives in focused docs while README surfaces the live dashboard demo.",
		run: async () => {
			const readme = await readRootText("README.md");
	...
	},
	{
		...
		run: async () => {
			return includesAll(`${await readRootText("README.md")}\n${await readText("skills/codex-autoresearch/SKILL.md")}`, [
				"AX",
				"AI experience",
				"UX",
				"user experience",
				"one skill surface",
				"live dashboard URL"
			]) ? pass() : fail("AX/UX golden path guidance is incomplete.");
		}
	},
	{
		...
		run: async () => {
			return includesAll(await readText("skills/codex-autoresearch/SKILL.md"), [
				"## Start Or Resume",
				"setup_plan",
				"setup_session",
				"doctor_session",
				...
				"commitPaths"
			]) ? pass() : fail("Main skill is missing setup, dashboard, continuation, or scoped Git guidance.");
		}
	},
	{
		...
		run: async () => {
			return includesAll(await readText("skills/codex-autoresearch/SKILL.md"), [
				...
				"finalize_preview",
				"Runway order"
			]) ? pass() : fail("Main skill is missing research, dashboard, or finalization guidance.");
		}
	},
	{
		...
		run: async () => {
			return includesAll(`${await readRootText("README.md")}\n${await readText("skills/codex-autoresearch/SKILL.md")}\n${await readText("scripts/autoresearch.ts")}\n${`${await readText("lib/mcp-interface.ts")}\n${await readText("lib/mcp-tool-schemas.ts")}`}`, [
				"continuation.shouldContinue",
				"continuation.forbidFinalAnswer",
				"loopContinuation",
				"active-loop continuation contract"
			]) ? pass() : fail("Missing active-loop continuation docs or CLI/MCP continuation output.");
		}
	},
	{
		...
		run: async () => {
			return includesAll(`${await readText("scripts/autoresearch.ts")}\n${`${await readText("lib/mcp-interface.ts")}\n${await readText("lib/mcp-tool-schemas.ts")}`}`, [
				"research-setup --cwd <project>",
				"quality-gap --cwd <project>",
				"setup_research_session",
				"measure_quality_gap",
				"METRIC quality_closed"
			]) ? pass() : fail("CLI/MCP research commands are not fully exposed.");
		}
	},
	{
		...
		run: async () => {
			return includesAll(await readText("assets/autoresearch.md.template"), [
				"## Decision Rules",
				"## Stop Conditions",
				"## Research Notes"
			]) ? pass() : fail("Session template lacks durable decision and stop-condition sections.");
		}
	},
	{
		...
		run: async () => {
			...
			return prompts.length <= 3 && prompts.every((prompt) => prompt.length < 128) && includesAll(promptText, [
			...
			]) && manifest.interface?.longDescription?.includes("one skill surface") ? pass() : fail("Default prompts should be concise plugin-level starters.");
		}
	},
	{
		id: "source-download-runtime",
		file: ".gitignore, dist/scripts/*.mjs, dist/lib/*.mjs, scripts/*.mjs",
		description: "Git/marketplace source downloads include the compiled TypeScript runtime used by the public launcher scripts.",
		run: async () => {
			const ignoredDist = (await readText(".gitignore")).split(/\r?\n/).map((line) => line.trim()).some((line) => line === "dist/" || line === "/dist/" || line === "dist");
			const required = [
			...
			const missing = [];
			for (const file of required) if (!await fileExists(file)) missing.push(file);
			return !ignoredDist && missing.length === 0 ? pass() : fail(ignoredDist ? "dist is ignored, so source downloads can miss the compiled runtime." : `Missing runtime files: ${missing.join(", ")}`);
		}
	},
	{
		...
		run: async () => {
			return includesAll(await readText(".gitignore"), [
				"/autoresearch.md",
				"/autoresearch.jsonl",
				"/autoresearch.config.json",
				"/autoresearch.ideas.md",
				"/autoresearch.research/"
			]) ? pass() : fail("Root session artifacts are not ignored.");
		}
	},
	{
		...
		run: async () => {
			return includesAll(await readText("scripts/finalize-autoresearch.ts"), [
				"autoresearch.research",
				"startsWith(`${RESEARCH_DIR}/`)",
				"session artifact verification"
			]) ? pass() : fail("Finalizer does not exclude autoresearch.research scratchpads.");
		}
	},
	{
		...
		run: async () => {
			...
			return String(pkg.scripts?.["check:product"] || "").includes("scripts/check.mjs") && checkScript.includes("perfection-benchmark.mjs") && checkScript.includes("--fail-on-gap") ? pass() : fail("package check does not run scripts/check.mjs with perfection-benchmark --fail-on-gap.");
		}
	},
	{
		...
		run: async () => {
			try {
				...
				return hasRegex(test, /quality_gap\s*=\s*0/) && test.includes("perfection-benchmark.mjs") ? pass() : fail("Test exists but does not assert the zero-gap metric.");
			} catch {
	...
	},
	{
		...
		run: async () => {
			...
			if (labelCount >= 4 && (template.includes("<label for=\"segment-select\">") || template.includes("htmlFor=\"segment-select\"") || template.includes("htmlFor:`segment-select`")) && template.includes("htmlFor=\"log-decision-status\"") && template.includes("htmlFor=\"log-decision-description\"") && template.includes("htmlFor=\"log-decision-asi\"") && template.includes("score-label") && template.includes("readout-label") && !template.includes("<label>Best kept change</label>")) return pass();
	...
	},
	{
		...
		run: async () => {
			return (await readText("assets/template.html")).includes("<link rel=\"icon\" href=\"data:image/svg+xml,") ? pass() : fail("Missing embedded data-URL favicon.");
		}
	},
	{
		...
		run: async () => {
			return includesAll(await readDashboardSurface(), [
				"chart-legend",
				"legend-swatch",
				"win-zone",
				"best-line",
				...
				"formatChartRunValue",
				"trend-chart-summary"
			]) ? pass() : fail("Dashboard chart is missing practical run-chart affordances.");
		}
	},
	{
		...
		run: async () => {
			const combined = `${await readDashboardSurface()}\n${await readText("lib/dashboard-view-model.ts")}`;
			return [
				"Codex will summarize",
				"Generated from runs, ASI",
				"Generated summary",
				"what happened and what it plans"
			].every((phrase) => !combined.includes(phrase)) && includesAll(combined, [
				"Current readout",
				"Next move",
				"Ledger, ASI"
			]) ? pass() : fail("Dashboard still contains placeholder or explanatory narration.");
		}
	},
	{
	...
	},
	{
		...
		run: async () => {
			return includesAll(`${await readText("scripts/autoresearch.ts")}\n${await readText("tests/autoresearch-cli.test.ts")}`, [
				"deleteLastRunPacket",
				"assertFreshLastRunPacket",
				"status is required; choose keep or discard explicitly",
				"stale last-run packets are rejected"
			]) ? pass() : fail("Last-run packet safety behavior is not implemented and tested.");
		}
	},
	{
		...
		run: async () => {
			...
			return includesAll(cli + cliHandlers + mcpInterface, [
				...
				"gap_candidates",
				"finalize_preview"
			]) ? pass() : fail("Missing one or more full-product CLI/MCP surfaces.");
		}
	},
	{
	...
	},
	{
		...
		run: async () => {
			return includesAll(await readText("tests/full-product.test.ts"), [
				"session core",
				...
				"live server",
				"serve_dashboard"
			]) ? pass() : fail("Missing focused full-product regression tests.");
		}
	}
];
const results = [];
...
} else console.log("No gaps found.");
...
if (process.argv.includes("--fail-on-gap") && failed.length > 0) process.exitCode = 1;
//#endregion
export {};
