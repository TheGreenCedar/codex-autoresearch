...
const toolSchemas = applyToolContracts([
	...
	{
		name: "prompt_plan",
		description: "Convert a natural-language Autoresearch request into inferred loop intent, missing essentials, setup defaults, and first safe commands.",
		inputSchema: {
			type: "object",
			properties: {
				working_dir: { type: "string" },
	...
	},
	{
		...
		description: "Return a compact human-and-agent onboarding packet with state, hazards, report templates, and next commands.",
		inputSchema: {
			type: "object",
			properties: {
				working_dir: { type: "string" },
	...
	{
		name: "recommend_next",
		description: "Return the single safest next action with why-it-is-safe evidence and copyable commands.",
		inputSchema: {
			type: "object",
			properties: {
				working_dir: { type: "string" },
	...
	},
	{
		...
		inputSchema: {
			...
			properties: {
				...
				keep_policy: {
					type: "string",
					enum: ["primary-only", "primary-or-risk-reduction"]
				},
	...
	{
		name: "new_segment",
		description: "Start a fresh run segment while preserving old ledger history; requires confirmation unless dry-run.",
		inputSchema: {
			type: "object",
			properties: {
				working_dir: { type: "string" },
...
function validateToolArguments(name, args, options = {}) {
	...
	for (const required of schema.required || []) if (normalized[required] == null || normalized[required] === "") throw new Error(`Missing required argument: ${required}`);
...
function requireUnsafeCommandGate(toolName, args, boolOption = defaultBoolOption) {
	...
	if (Boolean(normalized.command || normalized.benchmark_command || normalized.checks_command || normalized.model_command || setupCatalogCanMaterializeCommands) && !boolOption(normalized.allow_unsafe_command, false)) throw new Error(`${toolName} custom shell commands require allow_unsafe_command=true over MCP. Prefer a configured autoresearch script when possible.`);
}
...
function toMcpToolSchema(tool, options = {}) {
	const schema = {
		name: tool.name,
