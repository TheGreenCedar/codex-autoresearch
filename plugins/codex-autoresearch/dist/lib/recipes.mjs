...
//#region lib/recipes.ts
const PLUGIN_ROOT = resolvePackageRoot(import.meta.url);
const RECIPE_CATALOG_MAX_BYTES = 2 * 1024 * 1024;
const RECIPE_CATALOG_TIMEOUT_MS = 1e4;
const BUILT_IN_RECIPES = [
	{
		...
		metricUnit: "s",
		direction: "lower",
		benchmarkCommand: "node -e \"const {spawnSync}=require('node:child_process'); const c=process.platform==='win32'?'npm.cmd':'npm'; const r=spawnSync(c,['test'],{stdio:'inherit'}); process.exit(r.status ?? 1)\"",
	...
	},
	{
		...
		metricUnit: "points",
		direction: "higher",
		benchmarkCommand: "node -e \"console.error('Replace with lighthouse CLI JSON parsing, then print METRIC score=<number>'); process.exit(1)\"",
	...
	},
	{
		...
		metricUnit: "bytes",
		direction: "lower",
		benchmarkCommand: "node -e \"const fs=require('node:fs'); const p=process.env.AUTORESEARCH_BUNDLE_PATH||'dist'; let total=0; function walk(x){ if(!fs.existsSync(x)) return; const s=fs.statSync(x); if(s.isDirectory()) for(const f of fs.readdirSync(x)) walk(require('node:path').join(x,f)); else total+=s.size; } walk(p); console.log('METRIC bytes='+total);\"",
		benchmarkPrintsMetric: true,
	...
	},
	{
		...
		metricUnit: "s",
		direction: "lower",
		benchmarkCommand: "node -e \"console.error('Replace this recipe command with the real workload'); process.exit(1)\"",
		checksCommand: "",
		scope: ["scripts", "src"],
	...
	},
	{
		...
		metricUnit: "MB",
		direction: "lower",
		benchmarkCommand: "node -e \"console.log('METRIC rss_mb='+Math.round(process.memoryUsage().rss/1024/1024))\"",
		benchmarkPrintsMetric: true,
		checksCommand: "",
		scope: ["src", "scripts"],
	...
	},
	{
		...
		metricUnit: "s",
		direction: "lower",
		benchmarkCommand: "node -e \"console.error('Replace with a command that prints METRIC seconds=<number>'); process.exit(1)\"",
		benchmarkPrintsMetric: true,
		checksCommand: "",
		scope: ["src", "tests"],
...
function applyRecipeDefaults(args, recipeId) {
	if (!recipeId) return args;
	return applyRecipeObjectDefaults(args, recipeDefaultsForSetup(recipeId));
}
function applyRecipeObjectDefaults(args, defaults) {
	return {
		...args,
		...
		benchmarkPrintsMetric: args.benchmarkPrintsMetric ?? args.benchmark_prints_metric ?? defaults.benchmarkPrintsMetric,
		benchmark_prints_metric: args.benchmark_prints_metric ?? args.benchmarkPrintsMetric ?? defaults.benchmarkPrintsMetric,
...
async function findRecipe(id, catalog = null) {
	const builtIn = getBuiltInRecipe(id);
	if (builtIn) return builtIn;
	return (catalog ? await loadRecipeCatalog(catalog) : []).find((recipe) => recipe.id === id) || null;
}
async function applyResolvedRecipeDefaults(args, recipeId, catalog = null) {
	if (!recipeId) return args;
...
async function recommendRecipe(workDir) {
	...
	if (exists("package.json")) {
		...
		if (deps.typescript || exists("tsconfig.json")) return getBuiltInRecipe("typescript-compile-time");
	}
...
async function loadRecipeCatalog(catalog) {
	if (!catalog) return [];
	const text = /^https?:\/\//i.test(catalog) ? await fetchText(catalog) : await readBoundedCatalogFile(path.resolve(catalog));
	const parsed = JSON.parse(text);
	const recipes = Array.isArray(parsed) ? parsed : parsed.recipes;
	if (!Array.isArray(recipes)) throw new Error("Recipe catalog must be an array or an object with recipes[].");
	return recipes.map(validateExternalRecipe);
}
async function readBoundedCatalogFile(filePath) {
	if ((await fsp.stat(filePath)).size > RECIPE_CATALOG_MAX_BYTES) throw new Error(`Recipe catalog is too large; limit is ${RECIPE_CATALOG_MAX_BYTES} bytes.`);
	return await fsp.readFile(filePath, "utf8");
}
function validateExternalRecipe(recipe) {
	for (const field of [
		"id",
		"title",
		"metricName",
	...
	return await new Promise((resolve, reject) => {
		...
		const request = client.get(url, (res) => {
			if (res.statusCode < 200 || res.statusCode >= 300) {
				fail(/* @__PURE__ */ new Error(`HTTP ${res.statusCode} while fetching recipe catalog`));
				res.resume();
				return;
			}
			const contentLength = Number(res.headers["content-length"] || 0);
			if (Number.isFinite(contentLength) && contentLength > RECIPE_CATALOG_MAX_BYTES) {
				fail(/* @__PURE__ */ new Error(`Recipe catalog is too large; limit is ${RECIPE_CATALOG_MAX_BYTES} bytes.`));
				res.destroy();
				return;
			}
			let body = "";
		...
		request.setTimeout(RECIPE_CATALOG_TIMEOUT_MS, () => {
			request.destroy(/* @__PURE__ */ new Error(`Timed out fetching recipe catalog after ${RECIPE_CATALOG_TIMEOUT_MS}ms.`));
		});
	});
}
//#endregion
export { applyRecipeDefaults, applyRecipeObjectDefaults, applyResolvedRecipeDefaults, findRecipe, getBuiltInRecipe, listBuiltInRecipes, loadRecipeCatalog, recipeDefaultsForSetup, recipeDefaultsFromRecipe, recommendRecipe };
