//#region lib/commands/inspect.ts
function createInspectCommands(deps) {
	async function benchmarkLint(args) {
		...
		const metricName = deps.validateMetricName(args.metric_name || args.metricName || state.config.metricName || "metric");
		...
		if (!sample) {
			...
			if (command) {
				commandResult = await deps.runShell(command, workDir, timeoutSeconds, { retainMetricNames: [metricName] });
	...
	async function benchmarkInspect(args) {
		...
		const timeoutSeconds = Math.max(1, deps.numberOption(args.timeout_seconds ?? args.timeoutSeconds, 5));
		...
		const result = await deps.runShell(command, workDir, timeoutSeconds, { retainMetricNames: [state.config.metricName].filter(Boolean) });
		...
		return {
			...
			parsedMetrics,
			outputPreview: deps.headText(output || result.fullOutput || result.output || "", 30, 12e3),
	...
	async function checksInspect(args) {
		...
		const timeoutSeconds = Math.max(1, deps.numberOption(args.timeout_seconds ?? args.timeoutSeconds, 60));
		if (!command) return {
			ok: true,
			workDir,
			ranCommand: false,
		...
		return {
			...
			warnings,
			hints: checksInspectHints(),
			outputPreview: deps.headText(output, 50, 16e3),
			outputTruncated: Boolean(result.outputTruncated || result.fullOutputTruncated),
