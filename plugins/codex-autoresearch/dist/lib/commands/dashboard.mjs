...
//#region lib/commands/dashboard.ts
function createDashboardCommands(deps) {
	...
	async function exportDashboard(args) {
		...
		const sourceCwd = showcaseExport ? path.relative(deps.pluginRoot, workDir).replaceAll("\\", "/") || "." : workDir;
		const runtimeDrift = await deps.buildDriftReport({
		...
		const fullJson = deps.boolOption(args.json_full ?? args.jsonFull ?? args.full ?? args.verbose, false);
		const result = {
			ok: true,
			workDir,
			output,
	...
	async function serveDashboard(args) {
		...
		const serveResult = await deps.serveAutoresearch({
			...
			viewModel: async () => deps.dashboardViewModel(workDir, config, {
				deliveryMode: "live-server",
				liveUrl,
...
//#endregion
export { createDashboardCommands };
