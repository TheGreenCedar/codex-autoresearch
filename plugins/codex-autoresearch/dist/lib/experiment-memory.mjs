...
//#region lib/experiment-memory.ts
...
function buildExperimentMemory({ runs = [], direction = "lower", settings = {} } = {}) {
	const kept = [];
	const rejected = [];
	const nextActions = [];
	...
	for (const run of enriched) {
		...
		if (nextActionHint) nextActions.push({
			run: run.run,
			nextActionHint
		});
	...
	const warnings = [];
	if (runs.length && missingAsiRuns.length) warnings.push(`Runs missing ASI memory fields: ${missingAsiRuns.slice(-5).join(", ")}.`);
	...
	const diversityGuidance = lanePortfolio.find((lane) => lane.priority === "high" && lane.status !== "waiting") || lanePortfolio.find((lane) => lane.status === "ready") || lanePortfolio[0] || null;
	return {
		direction,
		kept,
		rejected,
...
function detectRepeatedHypothesis({ proposed = "", memory = {} } = {}) {
	...
	for (const item of candidates) {
		...
		if (previous === key || previous.includes(key) || key.includes(previous) || proposedFamily && previousFamily && (proposedFamily === previousFamily || proposedFamily.includes(previousFamily) || previousFamily.includes(proposedFamily))) return {
			matchedRun: item.run,
			status: item.status,
...
function summarizeFamilies(runs, direction) {
	...
	for (const run of runs) {
		...
		if (metric != null && (bestMetric == null || isBetter(metric, bestMetric, direction))) family.bestRun = compactFamilyRun(run);
		if (run.status === "keep" && metric != null && (bestKeptMetric == null || isBetter(metric, bestKeptMetric, direction))) family.bestKeptRun = compactFamilyRun(run);
	}
	const summarized = [...map.values()].map((family) => ({
		...family,
		exhausted: family.runs >= 3 && family.rejected >= Math.max(2, family.kept + 1)
	}));
	const limited = summarized.sort((a, b) => b.runs - a.runs || (b.latestRun?.run || 0) - (a.latestRun?.run || 0)).slice(0, 8);
	const incumbent = bestIncumbentFamily(summarized, direction);
	if (incumbent && !limited.some((family) => family.key === incumbent.key)) return [...limited.slice(0, 7), incumbent];
	return limited;
}
function detectPlateau({ runs, families, direction }) {
	const best = bestRun(runs.filter((run) => Number.isFinite(Number(run.metric))).filter((run) => run.status === "keep"), direction);
	...
	for (const run of recent) familyCounts.set(run.family.key, (familyCounts.get(run.family.key) || 0) + 1);
	...
	const detected = Boolean(best && runs.length >= 5 && runsSinceBest >= 4 && (repeatedFamilyRuns >= 3 || recentFailures >= 3));
	return {
		detected,
		...
		reason: detected ? `Plateau risk: ${runsSinceBest} runs since the best keep, with ${repeatedFamilyRuns} recent run(s) in ${repeatedFamily?.label || "one family"}.` : "",
		recommendation: detected ? "Force a distant scout or constraint-removal lane before another near-neighbor tweak." : "Continue balancing incumbent confirmation with fresh scouts."
	};
}
...
function bestIncumbentFamily(families, direction) {
	...
	for (const family of families) {
		...
		if (metric != null && (!best || bestMetric == null || isBetter(metric, bestMetric, direction))) best = family;
	}
	return best;
}
function familyForRun(run) {
	...
	const settingsKey = settingsSignature(asi.settings || asi.params || asi.parameters || asi.config);
	return {
		key: canonicalFamilyKey(explicit || settingsKey || asi.hypothesis || run.description || `run ${run.run}`),
...
function settingsSignature(value) {
	...
	const entries = Object.entries(value).filter(([key]) => !FAMILY_IGNORE_KEYS.has(String(key).toLowerCase())).sort(([a], [b]) => a.localeCompare(b));
	if (!entries.length) return "";
	return entries.map(([key, item]) => `${key}:${typeof item === "object" ? JSON.stringify(item) : String(item)}`).join("|");
}
function familyLabel(value) {
	const text = String(value || "Unlabeled family").replace(/\s+/g, " ").trim();
	return text.length > 44 ? `${text.slice(0, 41)}...` : text;
}
function canonicalFamilyKey(value) {
	return String(value || "").toLowerCase().replace(/\b(repeat|attempt|trial|seed|run)\s*[:=-]?\s*\d+\b/g, "$1 *").replace(/\br\s*\d+\b/g, "r *").replace(/\b\d+(?:\.\d+)?(?:k|m|b|ms|s|mb|gb|dim|d)?\b/g, "#").replace(/\b(keep|discard|baseline|regression|quality|new|leader|full|query|try|use|test)\b/g, " ").replace(/[^a-z0-9#*]+/g, " ").trim().slice(0, 96) || "unlabeled";
}
...
//#endregion
export { buildExperimentMemory, detectRepeatedHypothesis };
