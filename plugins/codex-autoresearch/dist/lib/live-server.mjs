...
const DASHBOARD_ACTIONS = new Map([
	...
	["gap-candidates", {
		className: "preview",
		allowedBodyKeys: ["researchSlug", "slug"]
	}],
	["finalize-preview", {
		className: "preview",
		allowedBodyKeys: []
...
async function serveAutoresearch(args) {
		...
		try {
			...
			if (req.method === "GET" && url.pathname === "/") {
				send(res, 200, "text/html; charset=utf-8", await dashboardHtml({
					actionNonce: actionsEnabled ? actionNonce : void 0,
					actionNonceHeader: actionsEnabled ? "X-Autoresearch-Action-Nonce" : void 0
				}));
				return;
			}
			...
			if (req.method === "POST" && url.pathname.startsWith("/actions/")) {
				...
				try {
					if (!actionsEnabled) {
						sendJson(res, actionErrorEnvelope(action, "Live dashboard actions are disabled. Use CLI or MCP for actions.", "actions_disabled"), 403);
						return;
					}
					const policy = DASHBOARD_ACTIONS.get(action);
					if (!policy || url.pathname !== `/actions/${action}`) {
						sendJson(res, actionErrorEnvelope(action, "Action is not allowed from the dashboard.", "action_forbidden"), 403);
						return;
					}
					const admission = validateActionRequest(req, { actionNonce });
					if (!admission.ok) {
						sendJson(res, actionErrorEnvelope(action, admission.error, admission.code), admission.status);
						return;
					}
					...
					const failure = error;
					sendJson(res, actionErrorEnvelope(action, failure.message || String(failure), failure.code || "dashboard_action_failed"), failure.statusCode || 500);
				}
				return;
			}
			sendJson(res, {
			...
			const failure = error;
			sendJson(res, actionErrorEnvelope("dashboard", failure.message || String(failure), failure.code || "dashboard_action_failed"), failure.statusCode || 500);
		}
	});
	...
	if (!address || typeof address === "string") throw new Error("Autoresearch live dashboard did not expose a numeric loopback port.");
	return {
		ok: true,
		workDir,
		port: address.port,
...
async function actionArgs(action, workDir, body) {
	...
	if (action === "doctor-explain") return [
		"doctor",
		"--cwd",
		workDir,
		"--explain"
	];
	if (action === "onboarding-packet") return [
		"onboarding-packet",
		"--cwd",
		workDir,
		"--compact"
	];
	if (action === "recommend-next") return [
		"recommend-next",
		"--cwd",
		workDir,
		"--compact"
	];
	if (action === "benchmark-lint") return [
		"benchmark-lint",
		"--cwd",
		workDir
	];
	if (action === "new-segment-dry-run") return [
		"new-segment",
		"--cwd",
		workDir,
		"--dry-run"
	];
	if (action === "setup-plan") return [
		"setup-plan",
		"--cwd",
		workDir
	];
	...
	if (action === "gap-candidates") return [
		"gap-candidates",
		"--cwd",
		workDir,
		"--research-slug",
	...
	if (action === "finalize-preview") return [
		"finalize-preview",
		"--cwd",
		workDir
	];
...
async function logActionArgs(action, workDir, body) {
	...
	if (body?.confirm !== action) throw new DashboardActionError(`Log actions require confirm="${action}".`, 400, "log_confirmation_required");
	await assertLastRunFingerprint(workDir, body.lastRunFingerprint);
	const description = String(body.description || "").trim();
	if (!description || description.startsWith("Describe the ")) throw new DashboardActionError("Log actions require a specific description.", 400, "log_description_required");
	const asi = normalizeAsi(status, body.asi);
	if (!asi.ok) throw new DashboardActionError(asi.error, 400, "log_asi_invalid");
	const args = [
		"log",
		"--cwd",
		workDir,
...
var DashboardActionError = class extends Error {
	code;
	statusCode;
...
function validateActionRequest(req, { actionNonce }) {
	if (req.headers[ACTION_NONCE_HEADER] !== actionNonce) return {
		ok: false,
		status: 403,
	...
	if (!/^(127\.0\.0\.1|localhost)(:\d+)?$/i.test(host)) return {
		ok: false,
		status: 403,
		code: "host_forbidden",
	...
	if (origin && !sameLoopbackOrigin(origin, host)) return {
		ok: false,
		status: 403,
	...
	if (!origin && referer) try {
		if (!sameLoopbackOrigin(new URL(referer).origin, host)) return {
			ok: false,
			status: 403,
	...
	if (!String(req.headers["content-type"] || "").toLowerCase().startsWith("application/json")) return {
		ok: false,
		status: 415,
	...
	if (Number.isFinite(contentLength) && contentLength > ACTION_BODY_MAX_BYTES) return {
		ok: false,
		status: 413,
		code: "body_too_large",
...
function validateActionBody(action, policy, body) {
	if (!body || typeof body !== "object" || Array.isArray(body)) throw new DashboardActionError("Dashboard action body must be a JSON object.", 400, "body_not_object");
	const allowed = new Set(policy.allowedBodyKeys || []);
	for (const key of Object.keys(body)) if (!allowed.has(key)) throw new DashboardActionError(`Dashboard action body field is not allowed: ${key}.`, 400, "body_field_forbidden");
	if (action === "gap-candidates") {
		for (const key of [
			...
			"command",
			"checks_command"
		]) if (Object.hasOwn(body, key)) throw new DashboardActionError(`Dashboard gap preview cannot accept ${key}.`, 400, "body_field_forbidden");
	}
}
function normalizeAsi(status, value) {
	if (!value || typeof value !== "object" || Array.isArray(value)) return {
		ok: false,
		error: "Log actions require ASI JSON object evidence."
	};
	const out = {};
	for (const key of [
		"hypothesis",
		...
		if (text) out[key] = text.slice(0, 4e3);
	}
	if (status === "keep" && (!out.hypothesis || !out.evidence)) return {
		ok: false,
		error: "Keep decisions require ASI hypothesis and evidence."
	};
	if (status !== "keep" && !out.evidence && !out.rollback_reason) return {
		ok: false,
...
async function assertLastRunFingerprint(workDir, submitted) {
	if (!submitted) throw new DashboardActionError("Log actions require the current last-run fingerprint.", 400, "last_run_fingerprint_required");
	const current = await currentLastRunFingerprint(workDir);
	if (!current) throw new DashboardActionError("No last-run packet is available to log.", 409, "last_run_missing");
	if (submitted !== current) throw new DashboardActionError("Last-run packet changed. Refresh the dashboard before logging.", 409, "last_run_stale");
}
async function currentLastRunFingerprint(workDir) {
	const candidates = [];
	const gitPath = await runProcess("git", [
		"rev-parse",
		"--git-path",
	...
	if (gitPath?.exitCode === 0 && gitPath.stdout.trim()) candidates.push(path.resolve(workDir, gitPath.stdout.trim()));
...
async function readJsonBody(req, maxBytes) {
	...
	for await (const chunk of req) {
		bytes += chunk.length;
		if (bytes > maxBytes) throw new DashboardActionError(`Dashboard action bodies are limited to ${maxBytes} bytes.`, 413, "body_too_large");
		body += chunk.toString("utf8");
	}
	if (!body.trim()) return {};
	try {
		const parsed = JSON.parse(body);
		if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new DashboardActionError("Dashboard action body must be a JSON object.", 400, "body_not_object");
		return parsed;
	} catch (error) {
		if (error instanceof DashboardActionError) throw error;
		throw new DashboardActionError(`Malformed dashboard action JSON: ${error.message}`, 400, "body_malformed_json");
	}
}
...
function actionResultEnvelope(action, cliArgs, result) {
	...
	return {
		...
		receipt: {
			ok,
			action,
			receiptId: createHash("sha256").update(`${action}:${Date.now()}:${result.commandDisplay}`).digest("hex").slice(0, 16),
			...
			nextStep: parsed?.continuation?.nextAction || parsed?.nextAction || (ok ? "Refresh complete." : "Inspect the action output before retrying.")
		},
