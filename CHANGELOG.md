...
- Updated the README install path for the `TheGreenCedar/AgentPluginMarketplace` catalogue and removed Codex-facing runbook/skill-contract details from the public README.
...
- Moved dashboard runtime bundles out of tracked source: `assets/dashboard-build/` is ignored and regenerated by package/check flows before packing.
- Reused the package-check extracted dashboard smoke in the release workflow, and made `prepack` the single release publish build path before `npm pack`.
